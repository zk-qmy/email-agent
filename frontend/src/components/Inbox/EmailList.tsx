            ...
            <div className="flex items-center gap-3 px-5 py-2.5 bg-white border-b border-border flex-shrink-0">
                <div className="flex gap-0.5 bg-bg border border-border rounded-lg p-0.5">
                    <button
                        ...
                        onClick={() =>
                            useStore.getState().setInboxFilter("inbox")
                        }
                    >
                        Inbox
                    </button>
...
function EmailItem({
    ...
    return (
        ...
        >
            <div
                className="w-9 h-9 rounded-full text-white text-[12px] font-semibold flex items-center justify-center flex-shrink-0 mt-0.25"
                style={{ backgroundColor: avatarColor(fromLabel) }}
            >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-0.5">
                    <span className="text-xs font-semibold text-text truncate max-w-[140px]">
                        {escHtml(trunc(fromLabel, 22))}
                    </span>
                    <span className="text-[11px] text-text-muted flex-shrink-0">
                        {formatDate(email.created_at)}
                    </span>
                </div>
                <div className="text-xs text-text whitespace-nowrap overflow-hidden text-ellipsis mb-0.5">
                    {escHtml(trunc(email.subject || "(no subject)", 40))}
                </div>
                <div className="text-[12px] text-text-muted whitespace-nowrap overflow-hidden text-ellipsis">
                    {escHtml(trunc(email.body || "", 60))}
                </div>
            </div>
            {isUnread && (
                <div className="w-1.75 h-1.75 rounded-full bg-primary flex-shrink-0 mt-1.5" />
            )}
        </div>
    );
}
