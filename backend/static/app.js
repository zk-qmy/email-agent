...
function getInputValue(sectionContent, field) {
    const input = sectionContent.querySelector(`.input-field[data-var="${field}"]`);
...
function interpolateEndpoint(endpoint, userId, sectionContent) {
    ...
    result = result.replace(/\{\{email_id\}\}/g, getInputValue(sectionContent, 'email_id'));
    result = result.replace(/\{\{draft_id\}\}/g, getInputValue(sectionContent, 'draft_id'));
    result = result.replace(/\{\{thread_id\}\}/g, getInputValue(sectionContent, 'thread_id'));
    return result;
}
...
function interpolateBody(body, userId, sectionContent) {
    ...
    let result = body;
    result = result.replace(/\{\{user_id\}\}/g, userId);
    result = result.replace(/\{\{email\}\}/g, getInputValue(sectionContent, 'email'));
    result = result.replace(/\{\{password\}\}/g, getInputValue(sectionContent, 'password'));
    result = result.replace(/\{\{username\}\}/g, getInputValue(sectionContent, 'username'));
    result = result.replace(/\{\{email_id\}\}/g, getInputValue(sectionContent, 'email_id'));
    result = result.replace(/\{\{recipient_email\}\}/g, getInputValue(sectionContent, 'recipient_email'));
    result = result.replace(/\{\{subject\}\}/g, getInputValue(sectionContent, 'subject'));
    result = result.replace(/\{\{body\}\}/g, getInputValue(sectionContent, 'body'));
    result = result.replace(/\{\{draft_id\}\}/g, getInputValue(sectionContent, 'draft_id'));
    result = result.replace(/\{\{thread_id\}\}/g, getInputValue(sectionContent, 'thread_id'));
    result = result.replace(/\{\{context\}\}/g, getInputValue(sectionContent, 'context'));
    result = result.replace(/\{\{date\}\}/g, getInputValue(sectionContent, 'date'));
    result = result.replace(/\{\{time\}\}/g, getInputValue(sectionContent, 'time'));
    result = result.replace(/\{\{message\}\}/g, getInputValue(sectionContent, 'message'));
    return result;
}
...
function displayResponse(result) {
    ...
    const statusColor = result.status >= 200 && result.status < 300 ? '#10b981' : '#ef4444';
